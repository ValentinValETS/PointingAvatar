...
namespace Oculus.Skinning.GpuSkinning
{
    ...
    internal sealed class OvrComputeMeshAnimator : IDisposable
    {
        ...
        public OvrComputeMeshAnimator(
            ...
            bool hasTangents,
            bool doubleBufferOutput,
            Action outputUpdatedCallback)
        {
            ...
            _outputUpdatedCallback = outputUpdatedCallback;
            ...
            CreateDynamicDataBuffer(numMeshVerts, numMeshMorphTargets, numMeshJoints, doubleBufferOutput);
            CreateOutputBuffers(numMeshVerts, hasTangents, doubleBufferOutput);
            GetShaderKernel(hasTangents, doubleBufferOutput);
        }
        ...
        private void CreateDynamicDataBuffer(int numMeshVerts, int numMeshMorphTargets, int numMeshJoints,
            bool doubleBufferOutput)
        {
            ...
            int sliceOutputSize = doubleBufferOutput ? SLICE_OUTPUT_STRIDE_BYTES : 0;
            ...
            bool doubleBufferOutput)
        {
            _positionOutputBuffer =
                new ComputeBuffer(
                    GetPositionOutputBufferSize(numMeshVerts, _outputPositionPrecision, doubleBufferOutput),
                    BAG_OF_UINTS_STRIDE,
                    ComputeBufferType.Structured);
            _frenetOutputBuffer = new ComputeBuffer(
                GetFrenetOutputBufferSize(numMeshVerts, hasTangents, doubleBufferOutput),
                BAG_OF_UINTS_STRIDE,
                ComputeBufferType.Structured);
        }
        ...
        {
            string kernelName;
            if (hasTangents && doubleBufferOutput)
            {
                kernelName = HAS_TANGENTS_DOUBLE_BUFFER_KERNEL_NAME;
            }
            else if (hasTangents)
            {
                kernelName = HAS_TANGENTS_KERNEL_NAME;
            }
            else if (doubleBufferOutput)
            {
                kernelName = DOUBLE_BUFFER_KERNEL_NAME;
            }
            else
            {
            ...
            Debug.Assert(_shader.HasKernel(kernelName));
            ...
            bool doubleBufferOutput)
        {
            ...
            int positionOutputSize = positionStrideBytes * numMeshVerts;

            if (doubleBufferOutput)
            {
                // Create 2x size outputs
                positionOutputSize *= 2;
            }

            return positionOutputSize / BAG_OF_UINTS_STRIDE;
        }

        private static int GetFrenetOutputBufferSize(int numMeshVerts, bool hasTangents, bool doubleBufferOutput)
        {
            const int FRENET_ATTRIBUTE_STRIDE_BYTES = 4; // only supporting 10-10-10-2 format for normal/tangents
            int frenetOutputSize = FRENET_ATTRIBUTE_STRIDE_BYTES * numMeshVerts * (hasTangents ? 2 : 1);
        ...
        private readonly Action _outputUpdatedCallback;
