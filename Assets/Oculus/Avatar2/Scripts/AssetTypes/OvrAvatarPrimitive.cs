...
#pragma warning restore CA2213 // Disposable fields should be disposed
                    ...
                    unsafe
                    {
                        for (uint subMeshIndex = 0; subMeshIndex < subMeshCount; subMeshIndex++)
                        {
                            ...
                            if (subMeshResult.IsSuccess())
                            {
                                ...
                                if ((inclusionType & subMeshInclusionFlags) == 0 && inclusionType != CAPI.ovrAvatar2EntitySubMeshInclusionFlags.None)
                                {
        ...
        private void InvokeOnMeshLoaded(Mesh sourceMesh)
        {
            OvrAvatarLog.LogInfo($"InvokeOnAvatarMeshLoaded", "", OvrAvatarManager.Instance);
            Profiler.BeginSample("OvrAvatarManager::InvokeOnAvatareshLoaded Callbacks");
            try
            {
                OvrAvatarManager.Instance.InvokeMeshLoadEvent(this, sourceMesh);
            }
            catch (Exception e)
            {
                OvrAvatarLog.LogException("OnAvatarMeshLoaded user callback", e, primitiveLogScope, OvrAvatarManager.Instance);
            }
            finally
            {
                Profiler.EndSample();
        ...
        private NativeArray<UInt16> CreateIndexBuffer(in CAPI.ovrAvatar2Primitive prim, out VertexRepacker repacker)
        {
            ...
            try
            {
                ...
                if (result.IsSuccess())
                {
                    ...
                    repacker = VertexRepacker.Create(ref triBuffer, in prim, _bufferVertexCount, out var packedIndices,
                        ct);
            ...
            unsafe
            {
                ...
                for (UInt32 iMorphTarget = 0; iMorphTarget < morphTargetCount; ++iMorphTarget)
                {
                    ...
                    try
                    {
                        ...
                        if (normalsAvailable)
                        {
                            normalsArray = new NativeArray<Vector3>((int)bufferVertexCount, _nativeAllocator,
                                _nativeArrayInit);
                            unsafe
                            {
                        ...
                        {
                            tangentsArray = new NativeArray<Vector3>((int)bufferVertexCount, _nativeAllocator,
                                _nativeArrayInit);
                            unsafe
                            {
        ...
        private void SetupSkin(ref MeshInfo meshInfo)
        {
            ...
            if (jointCount > 0)
            {
                using (var jointsInfoArray =
                    new NativeArray<CAPI.ovrAvatar2JointInfo>((int)jointCount, _nativeAllocator,
                        _nativeArrayInit))
                {
                    unsafe
                    {
                        ...
                        if (result.EnsureSuccess("ovrAvatar2Primitive_GetJointInfo", primitiveLogScope))
                        {
                            ...
                            {
                                var jointInfo = jointInfoBuffer + i;
        ...
        private readonly struct VertexRepacker
        {
                ...
                in uint vertexBufferSize, out NativeArray<UInt16> finalIndices, CancellationToken ct)
            {
                if (indices.Length > 0)
                {
                    int totalVerts;
                    bool needsRepacking;
                    var ranges = ProcessIndices(ref indices, out totalVerts, out finalIndices, in prim,
            ...
            private struct BufferRange
            {
                ...
                // ReSharper disable once PureAttributeOnVoidMethod
                [Pure]
                public void ShiftIndexBuffer(in NativeArray<UInt16> src, in NativeArray<UInt16> dest,
                    ref Int32 destOffset, ref int totalVerts)
                {
        ...
        private sealed class OvrAvatarGpuSkinnedPrimitiveBuilder : IDisposable
        {
            ...
            {
                OvrAvatarLog.AssertConstMessage(createPrimitivesTask == null, "recreating gpu and/or compute primitive",
                    primitiveLogScope);
            ...
            public OvrAvatarGpuSkinnedPrimitive BuildPrimitive(MeshInfo meshInfo, Int32[] joints, bool hasTangents)
            {
                ...
                var primitive = new OvrAvatarGpuSkinnedPrimitive(shortName, _meshInfo.vertexCount,
                    ...
                    () =>
                    {
                        ...
                        if (!_meshInfo.HasPendingPrimitives)
                        {
                            _meshInfo = null;
                        }
                    });
