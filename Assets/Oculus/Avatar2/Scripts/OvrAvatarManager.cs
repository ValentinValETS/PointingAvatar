...
Roughly equal to number of cores used")]
        ...
        */
        ...
        [Tooltip("Platform postfix used when loading glb files directly out of Streaming Assets (not in a .zip)" +
                 "\n\nThe full filename will be: (path)_(platform)(quality)(version)(extension), e.g. customavatar_riftv03.glb.zst")]
        [SerializeField]
        ...
        [SerializeField]
        private int assetVersionDefault = 3;

        [Tooltip("Version suffix (padded to 2 digits) when loading glb files directly out of Streaming Assets (not in a .zip)")]
        [SerializeField]
        private int assetVersionAndroid = 3;

        [Tooltip("Version suffix (padded to 2 digits) when loading glb files directly out of Streaming Assets (not in a .zip)")]
        [SerializeField]
        private int assetVersionQuest2 = 3;
        ...
        internal void InvokeMeshLoadEvent(OvrAvatarPrimitive prim, Mesh sourceMesh)
        {
            if (HasMeshLoadListener)
            {
                MeshData destMesh = new MeshData(
                    sourceMesh.name,
